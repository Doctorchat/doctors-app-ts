...
const zodIntlErrors = (issue: z.ZodIssueOptionalMessage) => {
  if (issue.code === z.ZodIssueCode.too_small) {
    if (issue.type === 'string') {
      return i18n.t("common:zod_string_min", { min: issue.minimum });
    } else if (issue.type === 'number') {
      return i18n.t("common:zod_number_min", { min: issue.minimum });
    } else if (issue.type === 'array') {
      return i18n.t("common:zod_array_min", { min: issue.minimum });
    }
  }
  
  if (issue.code === z.ZodIssueCode.too_big) {
    if (issue.type === 'string') {
      return i18n.t("common:zod_string_max", { max: issue.maximum });
    } else if (issue.type === 'number') {
      return i18n.t("common:zod_number_max", { max: issue.maximum });
    }
  }
